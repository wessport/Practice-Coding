...
colnames(discharge_daily_r967) <- c('Date','obsDis','simvyni967D_Dis','simvyni967U_Dis')

dis_daily_r967 <- xts(select(discharge_daily_r967,obsDis,simvyni967D_Dis,simvyni967U_Dis),order.by=discharge_daily_r967$Date)

dy_dis_daily_r967 <- dygraph(dis_daily_r967, main = 'Observed (Baseflow Removed) and Simulated Daily Total Streamflow - August 8 2017', group='ensync',height = 450, width = "100%") %>%
                        dyRangeSelector() %>%
...
stats <- data.frame(matrix(ncol = 5, nrow=2))
